...
Publisher.prototype.publish = function (headers, options) {
  ...
  stream._transform = function (file, enc, cb) {
    ...
    const matchGlob = function (filter, fileName) {
      if (!filter) {
        return false;
      }
      if (Array.isArray(filter)) {
        return filter.some((pattern) => micromatch.isMatch(fileName, pattern));
      }

      if (typeof filter === 'string') {
        return micromatch.isMatch(fileName, filter);
      }
      return false;
    };
    ...
    Object.keys(headers).forEach((header) => {
      if (isObject(headers[header])) {
        if (matchGlob(header, file.relative)) {
          Object.assign(file.s3.headers, headers[header]);
        }
      } else {
